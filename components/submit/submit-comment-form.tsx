...
import { submitCommentSchema, type SubmitCommentValues } from '@/lib/validation/submit-comment';

const MAX_FILE_SIZE = 5000000;
const ACCEPTED_IMAGE_TYPES = ["image/jpeg", "image/jpg", "image/png", "image/webp"];

export default function SubmitCommentForm({ code, prof }: { code: string; prof: string }) {
    const { isSignedIn, user } = useUser();
    const [image, setImage] = useState<File | null | undefined>(null);
    const [isSubmitting, setIsSubmitting] = useState(false);
    const form = useForm<SubmitCommentValues>({
        resolver: zodResolver(submitCommentSchema),
        defaultValues: {
            code: code.toUpperCase(),
            prof: prof.replaceAll("%20", " ").replaceAll('$', '/').toUpperCase(),
            attendance: '3',
            pre: '3',
            grade: 0,
            hard: 0,
    ...
    const submit = async (values: any) => {
        ...
        try {
            ...
            route.push(`/reviews/${code}/${prof}?reload=1`)
        }
        finally {
    ...
    return (
        <div className='max-w-screen-xl mx-auto p-10 md:p-20'>
            <div className='text-3xl antialiased mb-4'>
                Commenting on {prof.replaceAll("%20", " ").replaceAll('$', '/')} for {code}
            </div>
            <div>
                <Form {...form}>
